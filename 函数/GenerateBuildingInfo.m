  ...
  for j = 1:n
        switch type{j}
            case '住宅建筑'
                ...
                flexible_load(j)=(0.5+rand())*sum(residential_load_curve)*flexible_index;
                storage_capacity(j)=(0.5+rand())*storage_index*sum(residential_load_curve);
            case '办公建筑'
                load_curve{j} = (0.5+rand())*office_load_curve;
                pv_curve{j} = (0.5+rand())*office_pv_curve;
                flexible_load(j)=(0.5+rand())*sum(office_load_curve)*flexible_index;
                storage_capacity(j)=(0.5+rand())*storage_index*sum(office_load_curve);
            case '工业园区'
                load_curve{j} = (0.5+rand())*industrial_load_curve;
                pv_curve{j} = (0.5+rand())*industrial_pv_curve;
                flexible_load(j)=(0.5+rand())*sum(industrial_load_curve)*flexible_index;
                storage_capacity(j)=(0.5+rand())*storage_index*sum(industrial_load_curve);
            case '文化建筑'
                load_curve{j} =(0.5+rand())* cultural_load_curve;
                pv_curve{j} = (0.5+rand())*cultural_pv_curve;
                flexible_load(j)=(0.5+rand())*sum(cultural_load_curve)*flexible_index;
                storage_capacity(j)=(0.5+rand())*storage_index*sum(cultural_load_curve);
            case '商业建筑'
                load_curve{j} =(0.5+rand())* commercial_load_curve;
                pv_curve{j} =(0.5+rand())* commercial_pv_curve;
                flexible_load(j)=(0.5+rand())*sum(commercial_load_curve)*flexible_index;
                storage_capacity(j)=(0.5+rand())*storage_index*sum(commercial_load_curve);
            otherwise

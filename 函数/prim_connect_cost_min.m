...
%     y = rand(nodenums, 1) * 10; % 随机生成 Y 坐标（范围 0 到 10）
    ...
    [edge_weights,edge_weights_P]=assign_weights(n,edges,distMatrix,P);
    % 计算当前树的边权和
    edge_sum = sum(edge_weights);  % 计算边权和
    min_cost = edge_sum;
   
    adjacency_matrix = tree_to_adjacency_matrix(edges, n,edge_weights_P);
    % 可视化结果
%     figure;
%     hold on;
...
%     grid on;
    ...
    %% 边赋值的递归函数
    function [edge_weights,edge_weights_P]=assign_weights(n,tree,distance_matrix,P)
        P_max=zeros(n,1);
        L_price=zeros(n,1);
        node_values=P; 
        edge_weights = zeros(size(tree, 1), 1); % 初始化边权值
        edge_weights_P=zeros(size(tree, 1), 1); % 初始化边权值
        % 复制一份树的边关系
            remaining_tree = tree;  
         % 保存每条边在原始树中的索引
            ...
            if size(remaining_tree, 1) == 1
                ...
                P_max_onedge_hole=P_max_onedge*1000/(3*380*0.85);%将功率转换为孔径  
                L_price_one=P_max_onedge_hole;
                edge_weights(original_edge_index) =L_price_one*distance_matrix(remaining_edge(1,1), remaining_edge(1,2)); % 取两侧节点中较大的值
                edge_weights_P(original_edge_index)=P_max_onedge;
            else
           % 逐层处理叶子节点
            while size(remaining_tree, 1) > 1
                ...
                for i = 1:length(leaf_nodes)
                    ...
                    for j = 1:size(remaining_tree, 1)
                        if remaining_tree(j, 1) == leaf || remaining_tree(j, 2) == leaf
                            ...
                            P_max_hole(leaf)=P_max(leaf)*1000/(3*380*0.85);%将功率转换为孔径
                            L_price(leaf)=P_max_hole(leaf);
                            edge_weights(original_edge_index) =L_price(leaf)*distance_matrix(leaf, connected_node);
                            edge_weights_P(original_edge_index)=P_max(leaf);
                            % 将叶子节点的值加到相连节点上
                ...
                if size(remaining_tree, 1) == 1
                    ...
                    P_max_last_hole=P_max_last*1000/(3*380*0.85);
                    L_price_last=P_max_last_hole;
                    edge_weights(original_edge_index) =L_price_last*distance_matrix(remaining_edge(1,1), remaining_edge(1,2)); % 取两侧节点中较大的值
                    edge_weights_P(original_edge_index)=P_max_last;
                end
            end
    end

    %% 辅助函数：将树的边结构转换为0-1(最大功率)邻接矩阵
    function  adjacency_matrix = tree_to_adjacency_matrix(tree, n,edge_weights_P)
        % 初始化 n x n 的零矩阵
        ...
        for i = 1:size(tree, 1)
            node1 = tree(i, 1);
            node2 = tree(i, 2);
            weight=edge_weights_P(i);
            adjacency_matrix(node1, node2) = weight;
            adjacency_matrix(node2, node1) = weight;  % 对称矩阵，保证无向图
        end
    end
